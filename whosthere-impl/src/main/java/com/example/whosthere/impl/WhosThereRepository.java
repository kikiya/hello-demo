...
public class WhosThereRepository {
    ...
    public CompletionStage<Done> addGuest(String guest, String message) {
        return session().thenCompose(session ->
                session.executeWrite("INSERT INTO lagom_guests (guest, message) VALUES (?, ?)",
                        guest, message)
        );
    }

    public CompletionStage<Optional<List<Guest>>> getGuests() {
        return session().thenCompose(session ->
                session.selectAll("SELECT * FROM greeting_message ")
        ).thenApply(maybeRow -> maybeRow.stream().map(row -> new Guest(row.getString("message"), row.getString("guest"))));
    }
}
